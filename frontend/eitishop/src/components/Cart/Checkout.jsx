...
const Checkout = () => {
  ...
  // Ensure cart is load before proceeding
  useEffect(() => {
  ...
  const handlePaymentSuccess = async (details) => {
    try {
      await dispatch(payCheckout({ checkoutId, paymentDetails: details })).unwrap();
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-7xl mx-auto py-10 px-6 tracking-tighter">
      ...
      <div className="bg-white rounded-lg p-6">
        ...
        <form onSubmit={handleCreateCheckout}>
          ...
          <div className="mt-6">
            {!checkoutId ? (
              <button type="submit" className="w-full bg-primary hover:bg-[#bfcb7f] text-white py-3 rounded">
                Continue to Payment
              </button>
            ) : (
              <div>
